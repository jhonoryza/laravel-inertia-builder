...
export function AppFieldBuilder({field, value, setData, error, isProcessing}: FieldBuilderProps) {
    ...
    return (
        <div key={field.name} className={`'space-y-2' ${field.isInline ? 'flex items-center space-x-2' : ''}`}>
            <Label hidden={field.type === 'hidden'} htmlFor={field.name}>{field.label}</Label>
            ...
            {['text', 'email', 'number', 'password', 'hidden'].includes(field.type) && (
                <Input
                    id={field.name}
                    ...
                    onChange={handleChange}
                    placeholder={field.placeholder || ''}
                    className={cn(field.mergeClass, field.type === 'hidden' ? 'hidden' : '')}
                    disabled={field.isDisable || isProcessing}
                />
            )}
            ...
            {
                field.type === 'select' && field.multiple === false && field.searchable === false && (
            ...
            {
                field.type === 'select' && field.multiple && field.searchable === false && (
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button
                                variant="outline"
            ...
            {
                (field.type === 'combobox' && field.multiple === false)
                    || (field.type === 'select' && field.searchable && field.multiple === false) && (
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button
                                variant="outline"
...

            {
                (field.type === 'combobox' && field.multiple)
                    || (field.type === 'select' && field.searchable && field.multiple) && (
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button
                                variant="outline"
