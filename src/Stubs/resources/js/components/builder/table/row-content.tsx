...
export function AppDatatableRowContent({ data, columns, selectedIds, hiddenColumns, toggleSelectOne, children }: Props) {
        ...
        data.map((item: DataItem, index: number) => {
            ...
            return (
                <TableRow key={`table-row-${index}`} className={selectedIds.includes(id) ? 'bg-muted' : ''}>
                    ...
                    <TableCell>{children && children(item)}</TableCell>
                </TableRow>
            );
        })
    );
